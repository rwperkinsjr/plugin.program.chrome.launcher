...
addonFolder = xbmc.translatePath("special://home/addons/"+addonID+"/resources/thumbs/")
...
def addSiteDir(title, url, mode, thumb, stopPlayback, kiosk):
    thumb = addonFolder+thumb
    u = sys.argv[0]+"?url="+urllib.quote_plus(url)+"&mode="+urllib.quote_plus(mode)+"&thumb="+addonFolder+thumb+"&stopPlayback="+urllib.quote_plus(stopPlayback)+"&kiosk="+urllib.quote_plus(kiosk)
    ok = True
    fanart = thumb
    liz = xbmcgui.ListItem(title, iconImage=thumb, thumbnailImage=thumb)
    liz.setInfo(type="Video", infoLabels={"Title": title})
